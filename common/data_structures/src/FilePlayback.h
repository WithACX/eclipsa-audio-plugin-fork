...
class FilePlayback final : public RepositoryItemBase {
 ...
 private:
  ...
  EXPORT_VALUE(int, totalFileLength, TotalFileLength);
  EXPORT_VALUE(int, currentSecond, CurrentSecond);
  EXPORT_VALUE(CurrentPlayerState, playState, PlayState);
  EXPORT_VALUE(juce::String, audioElements, AudioElements);
  EXPORT_VALUE(juce::String, loudnessInfo, LoudnessInfo);
  EXPORT_VALUE(juce::String, currentMixPresentation, CurrentMixPresentation);
